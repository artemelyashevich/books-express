...
export class UserRepository {
    ...
    findAll(): Promise<TUserDto[]> {
        return new Promise((resolve, reject): void => {
            pool.query<TUserDto[]>('SELECT person.id, person.first_name, person.email, person.balance, person.last_name, role.name as role_name FROM email.person JOIN email.role ON person.role_id = role.id;',
                (err: QueryError | null, res: any) => {
                if (err) reject(err)
                else resolve(res)
            })
        })
    ...
    findByEmail(email: string): Promise<TUserDto> {
        return new Promise((resolve, reject): void => {
            pool.query<TUserDto[]>(
                `SELECT person.id, person.first_name, person.email, person.balance, person.last_name, role.name as role_name FROM person  JOIN role ON role.id = person.role_id WHERE person.email = ?`, [email],
                (err: QueryError | null, res: any) => {
                    if (err) reject(err)
                    else resolve(res[0])
                })
    ...
    findUser(email: string = '', id: Number = 0): Promise<TUserDto> {
        if (id == 0) {
            return new Promise((resolve, reject): void => {
                pool.query<TUserDto[]>(
                    `SELECT * FROM person WHERE person.email = ?`, [email], (err: QueryError | null, res: any) => {
                        if (err) reject(err)
                        else resolve(res[0])
                    })
            })
        ...
        return new Promise((resolve, reject): void => {
            pool.query<TUserDto[]>(
                `SELECT person.id, person.password_hash ,person.first_name, person.refresh_token, person.email, person.balance, person.last_name, role.name
 as role_name FROM person  JOIN role ON role.id = person.role_id WHERE person.id = ?`, [id],
                (err: QueryError | null, res: any) => {
                    if (err) reject(err)
                    else resolve(res[0])
                })
